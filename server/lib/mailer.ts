...
export async function sendPasswordResetEmail(email: string, otp: string, panelName: string = 'Survey Panel Go'): Promise<boolean> {
  const mailOptions = {
    from: `"Survey Panel Go" <${user}>`,
    to: email,
    ...
    html: `
      ...
      <head>
        ...
        <style>
          ...
          .otp-code { font-size: 36px; font-weight: 800; letter-spacing: 6px; color: #4338ca; margin: 0; font-family: monospace; }
          .footer { background: #fafafa; border-top: 1px solid #e2e8f0; padding: 20px; text-align: center; font-size: 12px; color: #64748b; }
          .warning { font-size: 13px; color: #ef4444; background: #fef2f2; padding: 12px 16px; border-radius: 8px; margin-top: 16px; border-left: 4px solid #ef4444; }
        </style>
      </head>
      <body>
        <div class="container">
          ...
          <div class="body">
            <p>Hello,</p>
            <p>We received a request to reset the password for your <strong>${panelName}</strong> account associated with <strong>${email}</strong>.</p>
            <p>Please use the one-time verification code below to authorize this password change:</p>
            ...
            <div class="warning">
              <strong>Security Notice:</strong> If you did not request a password reset, please ignore this email. Your current password remains secure.
            </div>
          </div>
          <div class="footer">
            &copy; ${new Date().getFullYear()} Survey Panel Go. All rights reserved.
