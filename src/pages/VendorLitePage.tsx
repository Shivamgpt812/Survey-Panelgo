  ...
  const createSurvey = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (surveyForm.type === 'internal') {
        ...
        const data = await response.json();
        if (data.success) {
          let link = data.link; // Generated by backend

          // Proactive Fix: Append placeholders if backend hasn't updated yet
          if (link && !link.includes('transactionId=')) {
            const sep = link.includes('?') ? '&' : '?';
            link += `${sep}transactionId=[TRANSACTION_ID]&userid=[USER_ID]`;
          }
